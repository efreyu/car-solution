...
case "$1" in
    ...
    --own|-o)
        ...
        ;;
    --wasm-build)
        eval ${DOCKERCORE} exec -it ubuntu bash build_in_container # cd /var/www && rm -rf public && mkdir public && cd public && source /etc/profile && cmake -DBUILD_TYPE=webassembly .. && make -j$(nproc)
        ;;
    --remove)
